...
| 통합 읽기 | `HWPLibReader` | `HWPFile` | HWP5·HWPML 지원, **HWP3는 변환기 미구현(예외)** |
| HWPML 읽기 | `HWPMLReader` | `HWPFile` | 기존 도구(TextExtractor 등) 그대로 재사용 |
| HWP 3.x 읽기 | `HWP3Reader` | `HWP3File` | 전용 모델. 텍스트 추출(`getText()`) 지원 |
...
kr.dogfoot.hwplib.object.hwp3       HWP3File, Hwp3Paragraph
...
```java
HWPFile hwp = HWPLibReader.fromFile("sample.hwp");     // HWP5 또는 HWPML
String text = TextExtractor.extract(hwp, TextExtractMethod.InsertControlTextBetweenParagraphText);
```

> ⚠️ **HWP 3.x 제한**: `HWPLibReader`는 HWP3를 `Hwp3ToHwpFileConverter`로 `HWPFile`로 변환해 반환하도록 설계됐으나, **변환기가 아직 미구현(P3/C6)**이라 HWP3 입력에 대해 `UnsupportedOperationException`을 던진다. HWP3 텍스트가 필요하면 아래 [HWP 3.x 읽기](#5-hwp-3x-읽기--hwp3reader)의 `HWP3Reader`를 직접 사용한다.
...
- ✅ 실문서 6종 HWP5 추출 대비 본문 96~99.9% 일치(본문/표/글상자 한글 누락 0)
- ⚠️ 글머리표/외곽번호 기호(• · □ ① 등)는 HWP3에서 문단 속성으로 생성되어 글자 스트림에 없어 미복원
- ⛔ 암호화된 HWP3 문서는 미지원(`hasPassword()==true`면 예외)
- ⛔ `HWPFile` 변환(`Hwp3ToHwpFileConverter`)·구조 보존 객체 모델은 미구현(P3/P4)
